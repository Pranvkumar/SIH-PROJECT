...
export default function LiveMap() {
  ...
  // Get translated text
  const liveMapText = useTranslatedText('Live Map');
  const gettingLocationText = useTranslatedText('Getting your location...');
  const locatingText = useTranslatedText('Locating you on the map...');
  const fewSecondsText = useTranslatedText('This may take a few seconds');
  const realTimeViewText = useTranslatedText('Real-time view of coastal hazard reports in your area');
  const approvedReportsText = useTranslatedText('approved reports shown');
  const hideReportsText = useTranslatedText('Hide Reports');
  const showReportsText = useTranslatedText('Show Reports');
  const showingDefaultText = useTranslatedText('Showing default location');
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg sm:text-xl">{liveMapText}</CardTitle>
            <CardDescription className="text-sm">{gettingLocationText}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-64 sm:h-80 md:h-96 bg-gray-50 rounded-lg flex items-center justify-center">
              <div className="text-center px-4">
                ...
                <p className="text-gray-600 mb-2 text-sm sm:text-base">{locatingText}</p>
                <p className="text-xs sm:text-sm text-gray-500">{fewSecondsText}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <CardDescription className="text-sm">
                {locationError ? (
                  <span className="flex items-center gap-2 text-amber-600">
                    ...
                    <span className="text-xs sm:text-sm">{locationError} {showingDefaultText}.</span>
                  </span>
                ) : (
                  `${realTimeViewText}. ${approvedReports.length} ${approvedReportsText}.`
                )}
              </CardDescription>
            </div>
            <Button
            ...
            >
              {showApprovedReports ? (
                <>
                  ...
                  <span className="text-xs sm:text-sm">{hideReportsText}</span>
                </>
              ) : (
                <>
                  <Eye className="w-3 h-3 sm:w-4 sm:h-4 mr-2" />
                  <span className="text-xs sm:text-sm">{showReportsText}</span>
                </>
              )}
            </Button>
          </div>
